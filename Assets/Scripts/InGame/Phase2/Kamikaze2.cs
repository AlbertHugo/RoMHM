...
public class Kamikaze2 : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (GlobalVariables.waveCounter >= 4 && GlobalVariables.waveCounter < 9)
        {
            speed = GlobalVariables.waveCounter - 3;
            baseSpeed = GlobalVariables.waveCounter - 3;
        ...
        else if (GlobalVariables.waveCounter >= 9 && GlobalVariables.waveCounter < 13&&speed<7)
        {
            speed = GlobalVariables.waveCounter - 7;
            baseSpeed = GlobalVariables.waveCounter - 7;
            trail.gameObject.SetActive(true);
            VisualEffect vfxTrail = GameObject.Instantiate(trail, kamiSelf.position, Quaternion.identity);
            vfxTrail.Play();
        ...
        }else if(GlobalVariables.waveCounter>=13&&speed<6)
        {
            speed = GlobalVariables.revengeCount - 12;
            baseSpeed = GlobalVariables.revengeCount - 12;
            trail.gameObject.SetActive(true);
            VisualEffect vfxTrail = GameObject.Instantiate(trail, kamiSelf.position, Quaternion.identity);
            vfxTrail.Play();
            GameObject.Destroy(vfxTrail.gameObject, (GlobalVariables.revengeCount-9) / 4);
            trail.gameObject.SetActive(false);
            vengeance.r = (GlobalVariables.revengeCount-9)/4;
            darkenedColor.r = (GlobalVariables.revengeCount - 9) / 4;
        }
        else
        {
            speed = 4.0f;
    ...
    void OnTriggerEnter(Collider collision)
    {
        if (GlobalVariables.waveCounter < 13)
        {
            ...
            {
                RepeatableCode.TakeDamage(ref life, 5, gameObject, deathExplosion, kamiSelf.position, kamiSelf.rotation, damageTaken, enemyDeathSound, hitSound);
            }
        }
        else
        {
    ...
    void HandleDarkeningEffect()
    {
        ...
        if (y > 3.7f && y < 6.2f)
        {
            if (GlobalVariables.waveCounter >= 4)
            {
